  ...
  COMMANDS: {
        ...
        MESSAGE_1: {
          ...
          FIELD_1: {
            ...
            VALUE: ({ version, memory }: { version: string; memory: string }) =>
              [
                `${Emojis.CIRCLE_CHEVRON_RIGHT} **Versión**: ${version}`,
                `${Emojis.CIRCLE_CHEVRON_RIGHT} **Uso de RAM**: ${memory}`,
              ].join("\n"),
          },
          FIELD_2: {
            FIELD: "**Estadísticas**",
            VALUE: ({ users, guilds, shards }: { users: number; guilds: number; shards: number }) =>
              [
                `${Emojis.CIRCLE_CHEVRON_RIGHT} **Usuarios**: ${users} usuarios`,
                `${Emojis.CIRCLE_CHEVRON_RIGHT} **Servidores**: ${guilds} servidores`,
        ...
        MESSAGE_1: {
          FIELD_1: {
            ...
            VALUE: ({ name, id, owner }: { name: string; id: string; owner: string }) =>
              [
                `${Emojis.CIRCLE_CHEVRON_RIGHT} **Nombre**: ${name}`,
                `${Emojis.CIRCLE_CHEVRON_RIGHT} **ID**: ${id}`,
                `${Emojis.CIRCLE_CHEVRON_RIGHT} **Propietario**: ${owner}`,
              ].join("\n"),
          },
          FIELD_2: {
            FIELD: "Estadísticas",
            VALUE: ({ members, channels, roles }: { members: number; channels: number; roles: number }) =>
              [
                `${Emojis.CIRCLE_CHEVRON_RIGHT} **Miembros**: ${members} miembros`,
                `${Emojis.CIRCLE_CHEVRON_RIGHT} **Canales**: ${channels} canales`,
        ...
        MESSAGE_1: {
          ...
          FIELD_1: {
            ...
            VALUE: ({ name, id }: { name: string; id: string }) =>
              [
                `${Emojis.CIRCLE_CHEVRON_RIGHT} **Usuario**: ${name}`,
                `${Emojis.CIRCLE_CHEVRON_RIGHT} **ID**: ${id}`,
              ].join("\n"),
          },
          FIELD_2: {
    ...
    MODERATION: {
      BAN: {
        MESSAGE_1: ({ user, moderator, reason }: { user: string; moderator: string; reason: string }) =>
          [
            `**${Emojis.CIRCLE_CHECK_COLOR} El usuario ${user} ha sido baneado por ${moderator}**`,
            `${Emojis.CIRCLE_CHEVRON_RIGHT} **Razón**: ${reason}`,
          ].join("\n"),
      },
      KICK: {
        MESSAGE_1: ({ user, moderator, reason }: { user: string; moderator: string; reason: string }) =>
          [
            `**${Emojis.CIRCLE_CHECK_COLOR} El usuario ${user} ha sido expulsado por ${moderator}**`,
            `${Emojis.CIRCLE_CHEVRON_RIGHT} **Razón**: ${reason}`,
          ].join("\n"),
      },
      PURGE: {
        NO_RECENT_MESSAGES: `**${Emojis.CIRCLE_X_COLOR} No hay mensajes recientes**`,
        MESSAGE_1: ({ messages }: { messages: number }) =>
          `**${Emojis.CIRCLE_CHECK_COLOR} Se han eliminado ${inlineCodeBlock(messages.toString())} mensajes**`,
      },
      SOFTBAN: {
        MESSAGE_1: ({ user, moderator, reason }: { user: string; moderator: string; reason: string }) =>
          [
            `**${Emojis.CIRCLE_CHECK_COLOR} El usuario ${user} ha sido softbaneado por ${moderator}**`,
            `${Emojis.CIRCLE_CHEVRON_RIGHT} **Razón**: ${reason}`,
          ].join("\n"),
      },
      TIMEOUT: {
        ADD: {
          INVALID_DURATION_FORMAT: `**${Emojis.CIRCLE_X_COLOR} La duración devolvió un formato inválido**`,
          ALLOWED_DURATION_VALUES: `**${Emojis.CIRCLE_X_COLOR} La duración debe ser mayor o igual a 5 segundos y menor o igual a 28 días**`,
          MESSAGE_1: ({ user, moderator, reason }: { user: string; moderator: string; reason: string }) =>
            [
              `**${Emojis.CIRCLE_CHECK_COLOR} El moderador ${moderator} ha añadido un tiempo de espera al usuario ${user}**`,
              `${Emojis.CIRCLE_CHEVRON_RIGHT} **Razón**: ${reason}`,
            ].join("\n"),
        },
        REMOVE: {
          USER_NOT_TIMEOUTED: `**${Emojis.CIRCLE_X_COLOR} El usuario no tiene un tiempo de espera**`,
          MESSAGE_1: ({ user, moderator, reason }: { user: string; moderator: string; reason: string }) =>
            [
              `**${Emojis.CIRCLE_CHECK_COLOR} El moderador ${moderator} ha eliminado el tiempo de espera del usuario ${user}**`,
              `${Emojis.CIRCLE_CHEVRON_RIGHT} **Razón**: ${reason}`,
            ].join("\n"),
        },
      },
      UNBAN: {
        BAN_NOT_FOUND: ({ ban }: { ban: string }) =>
          `**${Emojis.CIRCLE_X_COLOR} El baneo ${inlineCodeBlock(ban)} no ha sido encontrado**`,
        MESSAGE_1: ({ user, moderator, reason }: { user: string; moderator: string; reason: string }) =>
          [
            `**${Emojis.CIRCLE_CHECK_COLOR} El usuario ${user} ha sido desbaneado por ${moderator}**`,
            `${Emojis.CIRCLE_CHEVRON_RIGHT} **Razón**: ${reason}`,
          ].join("\n"),
      },
      WARN: {
        ADD: {
          MAX_WARNINGS_ALLOWED: [
            `**${Emojis.CIRCLE_X_COLOR} El usuario ya tiene el número máximo de advertencias**`,
            `${Emojis.CIRCLE_CHEVRON_RIGHT} **Límite**: 10 advertencias por usuario`,
          ].join("\n"),
          MESSAGE_1: ({ moderator, user, reason }: { moderator: string; user: string; reason: string }) =>
            [
              `**${Emojis.CIRCLE_CHECK_COLOR} El moderador ${moderator} ha añadido una advertencia al usuario ${user}**`,
              `${Emojis.CIRCLE_CHEVRON_RIGHT} **Razón**: ${reason}`,
            ].join("\n"),
        },
        REMOVE: {
          WARNING_NOT_FOUND: ({ id }: { id: string }) =>
            `**${Emojis.CIRCLE_X_COLOR} La advertencia ${inlineCodeBlock(id)} no ha sido encontrada**`,
          MESSAGE_1: ({ user, moderator, reason }: { user: string; moderator: string; reason: string }) =>
            [
              `**${Emojis.CIRCLE_CHECK_COLOR} El moderador ${moderator} ha eliminado una advertencia del usuario ${user}**`,
              `${Emojis.CIRCLE_CHEVRON_RIGHT} **Razón**: ${reason}`,
            ].join("\n"),
        },
        LIST: {
          WARNINGS_NOT_FOUND: ({ user }: { user: string }) =>
            `**${Emojis.CIRCLE_X_COLOR} El usuario ${user} no tiene advertencias**`,
          MESSAGE_1: {
            ...
            FIELD_1: {
              ...
              VALUE: ({ moderator, reason }: { moderator: string; reason: string }) =>
                [
                  `${Emojis.CIRCLE_CHEVRON_RIGHT} **Moderador**: ${moderator}`,
                  `${Emojis.CIRCLE_CHEVRON_RIGHT} **Razón**: ${reason}`,
                ].join("\n"),
            },
            FIELD_2: {
    ...
    UTILITY: {
      SUGGEST: {
        ...
        SYSTEM_NOT_ENABLED: `**${Emojis.CIRCLE_X_COLOR} El sistema de sugerencias no está habilitado**`,
        SUGGESTIONS_NOT_FOUND: `**${Emojis.CIRCLE_X_COLOR} El canal de sugerencias no ha sido encontrado**`,
        REVIEW_NOT_FOUND: `**${Emojis.CIRCLE_X_COLOR} El canal de revisión no ha sido encontrado**`,
        SUGGESTION_NOT_FOUND: ({ id }: { id: string }) =>
          `**${Emojis.CIRCLE_X_COLOR} La sugerencia ${inlineCodeBlock(id)} no ha sido encontrada**`,
        MESSAGE_1: {
        ...
        MESSAGE_2: `**${Emojis.CIRCLE_ALERT} Esta sugerencia requiere una aprobación antes de ser pública**`,
      },
    },
  },
  GLOBAL: {
