    ...
    run: async (_client: Discord, _context: CommandInteraction, { locale }) => {
        ...
        if (!guildConfiguration?.premium) {
            return await errorMessage({
                _context,
                ephemeral: true,
                message: Translations[locale].COMMANDS.CONFIGURATION.PREMIUM.REVOKE.INVALID_GUILD_MEMBERSHIP,
            });
        }

        const _originalMessageResponse = await _context.reply({
            embeds: new EmbedBuilder()
                .setDescription(Translations[locale].COMMANDS.CONFIGURATION.PREMIUM.REVOKE.MESSAGE_1)
        ...
        interactionCollector.on("collect", async (_collectedInteraction: AnyInteractionGateway) => {
            if (_collectedInteraction.isComponentInteraction()) {
                if (!(_collectedInteraction.inCachedGuildChannel() && _collectedInteraction.guild)) {
                    return await errorMessage({
                        _context,
                        ephemeral: true,
                        message: Translations[locale].GLOBAL.INVALID_GUILD_PROPERTY({
