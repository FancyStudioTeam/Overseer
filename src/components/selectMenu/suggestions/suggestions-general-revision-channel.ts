  ...
  run: async (
    ...
    if (channel?.type !== ChannelTypes.GUILD_TEXT) {
      return errorMessage(interaction, true, {
        description: client.locales.__({
          phrase:
            "commands.configuration.suggestions.row.general.row.revision-channel.invalid-channel-type",
          locale: language,
        }),
      });
    }
    ...
    const channels = {
      suggestion: interaction.guild.channels.get(guildSuggestion.channel_id),
      revision: interaction.guild.channels.get(channel.id),
    };

    if (
      channels.suggestion &&
      channels.revision?.id === channels.suggestion.id
    ) {
      return errorMessage(interaction, true, {
        description: client.locales.__({
          phrase:
            "commands.configuration.suggestions.row.general.row.revision-channel.cannot-be-same",
          locale: language,
        }),
      });
    }
    ...
    await prisma.guildSuggestion
      .update({
        ...
        data: {
          revision_channel_id: channel.id,
        },
      })
      .then(async (newData) => {
