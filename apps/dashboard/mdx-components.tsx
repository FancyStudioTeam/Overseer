...
export function useMDXComponents(): MDXComponents {
	...
	const sharedTextStyle = (className?: string) =>
		twMerge('text-neutral-400', className);

	return {
		h1: ({ children }) => (
			<h1 className={sharedHeadingStyle('text-3xl')}>{children}</h1>
		),
		h2: ({ children }) => (
			<h2 className={sharedHeadingStyle('text-2xl')}>{children}</h2>
		),
		h3: ({ children }) => (
			<h3 className={sharedHeadingStyle('text-xl')}>{children}</h3>
		),
		h4: ({ children }) => (
			<h4 className={sharedHeadingStyle('text-lg')}>{children}</h4>
		),
